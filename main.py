...
def get_historical_maintenance(db_path, start_date, end_date):
    ...
    try:
        ...
        logger.error(f"Errore nel caricamento delle manutenzioni: {str(e)}")
        return pd.DataFrame()
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            if table_exists.empty:
                logger.error("La tabella 'failures' non esiste nel database")
                return pd.DataFrame()
...
def get_historical_workload(db_path, start_date, end_date):
    ...
    try:
        ...
        logger.error(f"Errore nel caricamento del carico operativo: {str(e)}")
        return pd.DataFrame()
...
def get_historical_feedback(db_path, start_date, end_date):
    ...
    try:
        ...
        logger.error(f"Errore nel caricamento dei feedback: {str(e)}")
        return pd.DataFrame()
